...
// (References for Text and Audio generation are omitted for brevity, but exist in the full code)
...
`;
    ...
    try {
        ...
        symbolicOutput.textContent = generatedText;
        const notes = parseNoteString(generatedText);
        ...
        if (notes.length > 0) {
            ...
            symbolicStatus.textContent = 'LLM failed to generate valid note data. Please try again.';
        }
...
function parseNoteString(noteString: string): Note[] {
    ...
    const cleanedString = noteString.split('\n')[0];

    const entries = cleanedString.split(';');
    for (const entry of entries) {
        const parts = entry.trim().split(',');
        if (parts.length === 3) {
            const [note, octaveStr, durationStr] = parts;
            if (note.toUpperCase() === '---' || noteMap[note.toUpperCase().replace('#', 'S')] !== undefined) {
                 notes.push({
                    note: note.toUpperCase(),
                    octave: parseInt(octaveStr, 10) || 4,
                    duration: parseInt(durationStr, 10) || 8
                });
            }
        }
    }
...
function createXmFile(notes: Note[]): Blob {
    ...
    for(let i=0; i<sampleLength; ++i) {
        sampleData[i] = (i / sampleLength) * 254 - 127;
    }

    const patternRows = 64;
    let patternData = new Uint8Array(patternRows * 5); // 5 bytes per row: note, instr, vol, fx, fx param
    let currentRow = 0;

    for (const note of notes) {
        if (currentRow >= patternRows) break;

        const noteIndex = noteMap[note.note];
        if (note.note === '---') {
            // Note Off command
            patternData[currentRow * 5] = 97; // 97 is key-off
        } else if (noteIndex !== undefined) {
            patternData[currentRow * 5] = note.octave * 12 + noteIndex + 1;
            patternData[currentRow * 5 + 1] = 1; // Instrument 1
            patternData[currentRow * 5 + 2] = 64; // Max volume
        }
    ...
    const instrumentHeaderSize = 263; // Simplified size for 1 instrument
    const fileSize = headerSize + patternHeaderSize + patternData.length + instrumentHeaderSize + sampleLength;
    const buffer = new ArrayBuffer(fileSize);
    const view = new DataView(buffer);
    const textEncoder = new TextEncoder();

    // Helper to write strings
    const writeString = (offset: number, str: string, len: number) => {
        const encoded = textEncoder.encode(str);
        new Uint8Array(buffer, offset, len).set(encoded);
    };

    // Main XM Header
    writeString(view, 0, "Extended Module: ", 17);
    writeString(view, 17, "AI Generated Song", 20);
    view.setUint8(37, 0x1a);
    writeString(view, 38, "TypeScript LLM Tracker", 20);
    view.setUint16(58, 0x0104, true); // Version
    view.setUint32(60, headerSize - 60, true); // Header size
    view.setUint16(64, 1, true); // Song length in patterns
    view.setUint16(68, 1, true); // Number of instruments
    view.setUint16(72, 6, true); // Default speed
    view.setUint16(74, 125, true); // Default BPM
    view.setUint8(76, 0); // Pattern order table

    // Pattern Header
    let offset = headerSize;
    view.setUint32(offset, patternHeaderSize, true);
    view.setUint8(offset + 5, 1, true); // Number of rows in pattern
    view.setUint16(offset + 7, patternData.length, true);
    offset += patternHeaderSize;

    // Pattern Data
    new Uint8Array(buffer, offset).set(patternData);
    offset += patternData.length;

    // Instrument Header (1 instrument)
    view.setUint32(offset, instrumentHeaderSize, true);
    writeString(view, offset+4, "Sawtooth AI", 22);
    view.setUint8(offset+27, 1); // 1 sample in this instrument
    view.setUint32(offset+29, sampleLength, true); // sample header size
    // Point sample data to right after this header
    new Uint8Array(buffer, offset + 33).set(new Uint8Array(96)); // Note->Sample mapping
    new Uint8Array(buffer, offset + 129).set(new Uint8Array(48)); // Vol envelope
    new Uint8Array(buffer, offset + 177).set(new Uint8Array(48)); // Pan envelope
    offset += instrumentHeaderSize;

    // Sample Data
    new Int8Array(buffer, offset).set(sampleData);
...
async function handleMusicGeneration() {
    ...
    try {
        const result = (await musicPipe(musicPromptInput.value, {
            // progress_callback can be used here for more detailed feedback
        })) as AudioOutput;
...
function createWavBlob(audioData: Float32Array, sampleRate: number): Blob {
    ...
    // Write PCM data (converting from float to 16-bit PCM)
    let offset = 44;
...
// Add placeholders for brevity to make the code runnable
async function handleTextGeneration() { if (!textPipe || !textPromptInput.value) return; textGenerateButton.disabled = true; textStatus.textContent = 'Generating text...'; textOutput.textContent = ''; try { const result = await textPipe(textPromptInput.value, { max_new_tokens: 100 }); textOutput.textContent = result[0].generated_text; textStatus.textContent = 'Generation complete.'; } catch (error) { console.error('Text generation failed:', error); textStatus.textContent = 'Error during text generation.'; } finally { textGenerateButton.disabled = false; } }
async function handleMusicGeneration() { if (!musicPipe || !musicPromptInput.value) return; musicGenerateButton.disabled = true; saveWavButton.disabled = true; musicStatus.textContent = 'Generating music...'; audioOutput.src = ''; lastGeneratedAudio = null; try { const result = (await musicPipe(musicPromptInput.value)) as AudioOutput; lastGeneratedAudio = result; const wavBlob = createWavBlob(result.audio, result.sampling_rate); const audioUrl = URL.createObjectURL(wavBlob); audioOutput.src = audioUrl; musicStatus.textContent = 'Music generation complete!'; saveWavButton.disabled = false; } catch (error) { console.error('Music generation failed:', error); musicStatus.textContent = 'Error during music generation.'; } finally { musicGenerateButton.disabled = false; } }
function handleSaveWav() { if (!lastGeneratedAudio) return; const wavBlob = createWavBlob(lastGeneratedAudio.audio, lastGeneratedAudio.sampling_rate); const a = document.createElement('a'); a.href = URL.createObjectURL(wavBlob); a.download = 'generated-music.wav'; document.body.appendChild(a); a.click(); document.body.removeChild(a); }
function createWavBlob(audioData: Float32Array, sampleRate: number): Blob { const numChannels = 1; const bitsPerSample = 16; const byteRate = sampleRate * numChannels * (bitsPerSample / 8); const blockAlign = numChannels * (bitsPerSample / 8); const dataSize = audioData.length * numChannels * (bitsPerSample / 8); const chunkSize = 36 + dataSize; const buffer = new ArrayBuffer(44 + dataSize); const view = new DataView(buffer); const writeString = (view: DataView, offset: number, string: string) => { for (let i = 0; i < string.length; i++) { view.setUint8(offset + i, string.charCodeAt(i)); } }; writeString(view, 0, 'RIFF'); view.setUint32(4, chunkSize, true); writeString(view, 8, 'WAVE'); writeString(view, 12, 'fmt '); view.setUint32(16, 16, true); view.setUint16(20, 1, true); view.setUint16(22, numChannels, true); view.setUint32(24, sampleRate, true); view.setUint32(28, byteRate, true); view.setUint16(32, blockAlign, true); view.setUint16(34, bitsPerSample, true); writeString(view, 36, 'data'); view.setUint32(40, dataSize, true); let offset = 44; for (let i = 0; i < audioData.length; i++, offset += 2) { const s = Math.max(-1, Math.min(1, audioData[i])); view.setInt16(offset, s < 0 ? s * 0x8000 : s * 0x7FFF, true); } return new Blob([view], { type: 'audio/wav' }); }
