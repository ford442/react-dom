...
<canvas className='emscripten' id={'scanvas'} style={{pointerEvents:'auto',display:'block',position:'absolute',zIndex:3000,backgroundColor:'rgba(233,233,233,1.0)',top:'0',height:'100vh',width:'100vh',imageRendering:'auto',transform:'scaleY(1.0)'}}></canvas>
          ...
          {/* Main Floating Control Panel - All UI sections will now live inside this container */}
          <div className="base-panel floating-control-panel">
            
            {/* Status Message - Placed at the top of the panel */}
            <div className="status-display">
              {statusMessage}
            </div>
            
            {/* --- Personality and Engine Selection --- */}
            ...
            {/* --- Text Generation and STT Section --- */}
            <div className="panel-section">
              <h3>Text Generation & Voice Control</h3>
              <div className="input-group">
            ...
            <div className="panel-section">
              <h3>Image Captioning (ViT-GPT2)</h3>
              <div className="input-group">
            ...
            <div className="panel-section">
                ...
                <button
                    onClick={async () => {
                        if (activeTtsEngine === 'kokoro') await synthesizeWithKokoroAndPlay(textToSpeakInput, currentPersonalityKey);
                        else if (activeTtsEngine === 'speechT5') await synthesizeAndPlayText(textToSpeakInput);
                        else speakWithWebAPI(textToSpeakInput);
                    }}
                    disabled={isSpeaking || !textToSpeakInput.trim()}
                >
                    {isSpeaking ? 'Synthesizing...' : `Speak with ${activeTtsEngine}`}
                </button>
            </div>
            
          </div> {/* End of floating-control-panel */}
