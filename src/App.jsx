...
function App() {
    ...
    try {
      ...
      if (currentPuzzle.image && imageCaptioner) {
        ...
        if (captions && captions.length > 0) {
          promptForLLM += ` The image shows: ${captions[0].generated_text}`;
        }
      }
  ...
  }, [generator, imageCaptioner, currentPuzzleIndex, puzzleSet]);
  ...
  useEffect(() => {
    ...
    if (puzzleDataElement) {
      try {
        const puzzles = JSON.parse(puzzleDataElement.textContent);
        setPuzzleSet(puzzles);
        console.log("Loaded puzzles from HTML:", puzzles);
      } catch (error) {
        console.error("Failed to parse puzzle data from HTML:", error);
              ...
              <div className="panel-section">
                ...
                {puzzleSet.length > 0 ? (
                  <div className="puzzle-container">
                    <h4>{puzzleSet[currentPuzzleIndex].question}</h4>
                    {puzzleSet[currentPuzzleIndex].image && (
                      <img
                        src={puzzleSet[currentPuzzleIndex].image}
                        alt="Puzzle"
                        style={{ maxWidth: '100%', borderRadius: '8px', marginBottom: '1rem' }}
                      />
                    )}
                    <button onClick={handleSolvePuzzle} disabled={isSolvingPuzzle || !generator}>
                      {isSolvingPuzzle ? 'Solving...' : 'Get Answer'}
                    </button>
                  </div>
                ) : <p>Loading puzzles...</p>}
