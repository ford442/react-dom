...
// --- Helper functions moved outside (so MemoizedSVGCell can use them) ---
...
// --- New Memoized SVG Cell Component ---
interface MemoizedSVGCellProps {
  stepIdx: number;
  cellNote: string;
  isActive: boolean;
  baseGlobal: number;
...
const MemoizedSVGCell = React.memo(function SVGCell({
                                                      stepIdx,
                                                      cellNote,
                                                      isActive,
                                                      baseGlobal,
...
// --- Your Original PatternSequencer (Slightly Modified) ---
...
export const PatternSequencer: React.FC<PatternSequencerProps> = ({ matrix, currentRow, globalRow = 0, totalRows: _totalRows = 0, onSeek, bpm: _bpm = 120 }) => {  // 1. Defined all hooks unconditionally at the top
...
// Helpers (not hooks)
  ...
  const patternTiles = useMemo(() => {
    ...
    // Ensure patternRows is correctly typed or initialized
    const patternRows = matrix.rows || Array.from({ length: patternLen }, () =>
        Array.from({ length: columns }, () => ({ type: 'empty', text: '' } as PatternCell))
    );
  ...
  const svgWidth = labelWidth + patternLen * (cellWidth + cellGap) + 100;
  ...
  return (
      <section className="bg-gradient-to-b from-black/60 via-gray-900/60 to-black/40 p-4 rounded-xl mb-4 border border-white/5 shadow-2xl">
        ...
        <div className="mb-4 flex flex-col gap-3 relative">
          ...
          <svg width="100%" height={totalHeight} xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${svgWidth} ${totalHeight}`} preserveAspectRatio="xMidYMin meet">
            {/* ... (defs, header text as before) ... */}
            <defs>
              <filter id="neonGlow">
            ...
            <text x={svgWidth - 20} y="20" fill="#6b7280" fontSize="12" textAnchor="end">
            ...
            <g transform={`translate(0, ${headerHeight})`}>
              {Array.from({ length: columns }).map((_, chIdx) => {
                ...
                return (
                    <g key={chIdx} transform={`translate(0, ${yPos})`}>
                      ...
                      {/* Step cells: *** THIS IS THE CHANGED PART *** */}
                      {Array.from({ length: patternLen }).map((_, stepIdx) => {
                        const cells = patternRows[stepIdx] || Array.from({ length: columns }, () => ({ type: 'empty', text: '' } as PatternCell));
              ...
              {/* Playhead sweep line (no changes) */}
              <line
